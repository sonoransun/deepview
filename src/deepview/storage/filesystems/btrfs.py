"""Btrfs adapter via ``pyfsbtrfs`` (libfsbtrfs)."""
...
def _import() -> Any:
    try:
        import pyfsbtrfs  # type: ignore[import-not-found]
    except ImportError as exc:  # pragma: no cover
        raise StorageError("pyfsbtrfs not installed") from exc
    return pyfsbtrfs
...
class BtrfsFilesystem(Filesystem):
    ...
    def __init__(self, layer: DataLayer, offset: int = 0) -> None:
        ...
        try:
            ...
            raise StorageError(f"pyfsbtrfs open failed: {exc}") from exc
        self._volume = v
    ...
    def list(self, path: str = "/", *, recursive: bool = False, include_deleted: bool = False) -> Iterator[FSEntry]:
        try:
            ...
            raise StorageError(f"pyfsbtrfs path {path!r}: {exc}") from exc
    ...
    def stat(self, path: str) -> FSEntry:
        try:
            ...
            raise StorageError(f"pyfsbtrfs stat {path!r}: {exc}") from exc
    ...
    def open(self, path: str) -> DataLayer:
        try:
            ...
            raise StorageError(f"pyfsbtrfs open {path!r}: {exc}") from exc
