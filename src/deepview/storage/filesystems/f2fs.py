"""F2FS adapter via ``pyfsf2fs`` (libfsf2fs)."""
...
def _import() -> Any:
    try:
        import pyfsf2fs  # type: ignore[import-not-found]
    except ImportError as exc:  # pragma: no cover
        raise StorageError("pyfsf2fs not installed") from exc
    return pyfsf2fs
...
class F2FSFilesystem(Filesystem):
    fs_name = "f2fs"
    ...
    def __init__(self, layer: DataLayer, offset: int = 0) -> None:
        ...
        try:
            ...
            raise StorageError(f"pyfsf2fs open failed: {exc}") from exc
        self._volume = v
    ...
    def list(self, path: str = "/", *, recursive: bool = False, include_deleted: bool = False) -> Iterator[FSEntry]:
        try:
            ...
            raise StorageError(f"pyfsf2fs path {path!r}: {exc}") from exc
    ...
    def stat(self, path: str) -> FSEntry:
        try:
            ...
            raise StorageError(f"pyfsf2fs stat {path!r}: {exc}") from exc
    ...
    def open(self, path: str) -> DataLayer:
        try:
            ...
            raise StorageError(f"pyfsf2fs open {path!r}: {exc}") from exc
