"""XFS adapter via ``pyfsxfs`` (libfsxfs)."""
...
def _import() -> Any:
    try:
        import pyfsxfs  # type: ignore[import-not-found]
    except ImportError as exc:  # pragma: no cover
        raise StorageError("pyfsxfs not installed") from exc
    return pyfsxfs
...
class XFSFilesystem(Filesystem):
    ...
    def __init__(self, layer: DataLayer, offset: int = 0) -> None:
        ...
        try:
            ...
            raise StorageError(f"pyfsxfs open failed: {exc}") from exc
        self._volume = v
    ...
    def list(self, path: str = "/", *, recursive: bool = False, include_deleted: bool = False) -> Iterator[FSEntry]:
        try:
            ...
            raise StorageError(f"pyfsxfs path {path!r}: {exc}") from exc
    ...
    def stat(self, path: str) -> FSEntry:
        try:
            ...
            raise StorageError(f"pyfsxfs stat {path!r}: {exc}") from exc
    ...
    def open(self, path: str) -> DataLayer:
        try:
            ...
            raise StorageError(f"pyfsxfs open {path!r}: {exc}") from exc
