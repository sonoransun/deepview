"""HFS+ adapter via ``pyfshfs`` (libfshfs)."""
...
def _import() -> Any:
    try:
        import pyfshfs  # type: ignore[import-not-found]
    except ImportError as exc:  # pragma: no cover
        raise StorageError("pyfshfs not installed") from exc
    return pyfshfs
...
class HFSFilesystem(Filesystem):
    ...
    def __init__(self, layer: DataLayer, offset: int = 0) -> None:
        ...
        try:
            ...
            raise StorageError(f"pyfshfs open failed: {exc}") from exc
        self._volume = v
    ...
    def _to_entry(self, path: str, f: Any) -> FSEntry:
        return FSEntry(
            ...
            ctime=float(getattr(f, "entry_modification_time_as_integer", 0) or 0),
    ...
    def list(self, path: str = "/", *, recursive: bool = False, include_deleted: bool = False) -> Iterator[FSEntry]:
        try:
            ...
            raise StorageError(f"pyfshfs path {path!r}: {exc}") from exc
    ...
    def stat(self, path: str) -> FSEntry:
        try:
            ...
            raise StorageError(f"pyfshfs stat {path!r}: {exc}") from exc
    ...
    def open(self, path: str) -> DataLayer:
        try:
            ...
            raise StorageError(f"pyfshfs open {path!r}: {exc}") from exc
